...
ALLOWED_ADDR = ['127.0.0.1', '74.55.51.162', '94.175.121.72',]
...
def player_update():
    ...
    server.changeChannelGroup(clientid, team["channelid"], CHANNEL_GROUPS[rank["channelGroup"]]["groupid"])
...
def player_delete():
    ...
    for team in TEAMS:
        ...
        for rank in RANKS:
            ...
            server.removeFromServerGroup(clientid, rank["groupIdbyTeam"][team["id"]])
            if "additionalgroups" in rank:
                for groupid in rank["additionalgroups"][team["id"]]:
                    server.removeFromServerGroup(clientid, groupid)
            server.changeChannelGroup(clientid, team["channelid"], CHANNEL_GROUPS[0]["groupid"])
...
def set_battlemode():
    ...
    if data["state"] == "on":
        ...
        server.sendServerMessage(team["name"] + " channels are now in BATTLEMODE")

    if data["state"] == "off":
        server.setSubscribePower(team["channelid"], 0)
        server.sendServerMessage(team["name"] + " channels are no longer in BATTLEMODE")
